...
`expCompleted(W)` — the export's "finished slab" — currently stops at `paintStoneContent`. It does **not** bake the coat tier, back light, or lens; those run only in live `draw()`. So today's `render.png` is missing everything from v49 on. **Job one of Final Render is a full-fidelity offline render** that runs the complete pipeline (base → content → emission → subsurface → specular → lens, or the back-light fold) to an arbitrary target at arbitrary resolution.

## Build order

1. **`renderFull` — the offline render primitive.** The complete pipeline to a target canvas at resolution W, no pan/zoom, coat included. Factor the coat passes (currently inline in `draw()`, bound to `cv`/`ctx`/`dpr`) into a shared `applyCoat(targetCanvas, targetCtx, blurScale, flags)` used by *both* live `draw()` and `renderFull`, so there is one implementation and no drift. Blur/scatter radii scale with the output resolution, not the screen. **This is also what one tile renders**, so it is the foundation for everything below.
2. **Tiling + bleed.** Huge finals should not need one enormous framebuffer. Split the output into tiles; render each as a slab sub-region **expanded by a bleed margin** so cross-tile effects (blur, scatter, glow, refraction, displacement) are correct, then crop the bleed and assemble. Bounds RAM/VRAM — appropriate for constrained hardware (e.g. RX 6500 XT). Intermediate **resolution belongs to the operation**, not the export: a smooth optical field needs a few hundred samples across an axis and is interpolated up; only the final raster is enormous. (The Fresnel lens already proves this — real per-pixel gather over ~256k working pixels, upscaled.)
